...
int main() {
	...
	resize(input1, input1, Size(input1.cols / 4, input1.rows / 4));
	resize(input2, input2, Size(input2.cols / 4, input2.rows / 4));
	//TEMPORARY IMAGE TO CHECK MATCHED PAIRS
	Mat input1_1 = imread("C:\\input1.jpg", CV_LOAD_IMAGE_COLOR);
	Mat input2_1 = imread("C:\\input2.jpg", CV_LOAD_IMAGE_COLOR);
	resize(input1_1, input1_1, Size(input1_1.cols / 4, input1_1.rows / 4));
	resize(input2_1, input2_1, Size(input2_1.cols / 4, input2_1.rows / 4));
	...
	input1_1.copyTo(matchingImage(Rect(size.width, 0, input1_gray.size().width, input1_gray.size().height)));
	input2_1.copyTo(matchingImage(Rect(0, 0, size.width, size.height)));
