...
int main() {
	...
	input2_1.copyTo(matchingImage(Rect(size.width, 0, input1_gray.size().width, input1_gray.size().height)));
	input1_1.copyTo(matchingImage(Rect(0, 0, size.width, size.height)));
	...
	Mat A12 = cal_affine(dstPoints, srcPoints, srcPoints.size());
	Mat A21 = cal_affine(srcPoints, dstPoints, dstPoints.size());
