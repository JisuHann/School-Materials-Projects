...
class Loader(Dataset):
    ...
    def VOCdataloader(self, index):
        ...
        print(path1)
        img1 = cv2.cv2.imread(path1, cv2.IMREAD_COLOR)
        img1 = cv2.cv2.cvtColor(img1, cv2.COLOR_BGR2RGB)
    ...
    def __init__(self, VOC2012_dir, flag, resize, transforms):
        ...
        if self.flag == 'train':
            self.imgnames = self.lines[:500] # Tip : you can adjust the number of images and run the quickly during debugging.
            #self.imgnames = self.lines[self.fold:]

        else:
            self.imgnames = self.lines[500:600] # Tip : you can adjust the number of images and run the quickly during debugging.
            #self.imgnames = self.lines[:self.fold]
